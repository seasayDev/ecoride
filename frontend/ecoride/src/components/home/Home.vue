<template>
    <div class="container">
      <div class="row">
        <ScooterCard v-for="(scooter, index) in state.scooters" :key="index" :class="gridClass" :scooter="scooter" @reserve="openModal(scooter)">
        </ScooterCard>
      </div>
      <ReservationModal :isVisible="state.showModal" :trotinette="state.selectedTrotinette" @close="closeModal" @reserved="handleReservation"/>
    </div>
  </template>
